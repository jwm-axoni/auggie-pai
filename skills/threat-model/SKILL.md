...
| claude-browser MCP | Review application UI flows, inspect configurations via Axonius internal tools |
| System commands | Verify configurations, check network topology if authorized |

## Axonius Context

When threat modeling Axonius systems or features:
- Consider the multi-tenant SaaS architecture
- Pay attention to adapter/plugin trust boundaries (Axonius connects to many external systems)
- Model API key and credential storage as high-value assets
- Consider supply chain risks from third-party integrations
- Reference Axonius security documentation via docs.axonius.com through claude-browser
- Check Atlas/Jira for existing security architecture decisions
